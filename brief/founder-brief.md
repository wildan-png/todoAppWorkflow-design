...
- **What are the must-have features for launch (MVP)?**  
  Three main surfaces: **(1) Task list** — see everything in flight, add quickly, open for detail. **(2) Board by status** — same tasks, grouped by status. **(3) Task detail** — editable **description / notes**, **image attachments**, **priority**, **assignee / user assignment**, and **other core fields needed for the MVP** (exact list of “other” fields not fully enumerated). **Desktop / fixed-layout MVP** for this phase.
...
- **Has a tech stack been decided?**  
  ⚠️ Not mentioned — needs follow-up (brief positions this as **frontend ready to integrate** when backend/API exists; no stack named).

- **Are there required integrations? (APIs, third-party tools)**  
  ⚠️ Not mentioned — needs follow-up (open questions include **backend / API contract and auth**).
...
- **Who are the stakeholders involved in design decisions?**  
  **Client named Wildan** (contact: wildan). No explicit list of other decision-makers.

---

## Gaps called out in source (not expanded here)

From the original brief’s **Open questions**: backend/API contract and auth; image storage (upload flow, limits); exact default statuses/columns on the board.
